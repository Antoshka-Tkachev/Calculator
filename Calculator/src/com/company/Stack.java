...
class Stack implements Priority_operations {
    ...
    private ArrayList <Double> digits = new ArrayList<>();     // стек для чисел
    ...
    private double operate(double first, double second, String operation) { // возвращает результат, выполненной операции
        switch (operation) {
            case "+":
                return first + second;
            case "-":
    ...
    double result() {        // считает выражение и возвращает ответ
        String givenElement; // буфер для данного элемента
        double first;        // предыдущий элемент "стека"
        double second;       // последний элемент "стека"
        ...
        for (String s : formulaStr) { // от первого до последенго элемента
            ...
            switch (givenElement) {
                ...
                case ")":
                    ...
                    break;
                default:
                    digits.add(Double.parseDouble(givenElement));       // если это не операция, то данный элемент добавляем в стек чисел
                    break;
            }
        }
        ...
        double result = digits.get(0); // в результат записываем первое (единственное) число в стеке чисел
